...
void	close_middle_pipe(t_p_data p_data, t_word_list **word_list,
		t_env_list **env_list, t_here_list **here_list)
{
	if (close(p_data.pipe_2darr[p_data.i - 1]) < 0)
	{
		ft_perror("close");
		free_list_pipe2darr_exit(&p_data.pipe_2darr, word_list, env_list,
			here_list);
	}
	if (close(p_data.pipe_2darr[p_data.i]) < 0)
	{
		ft_perror("close");
		free_list_pipe2darr_exit(&p_data.pipe_2darr, word_list, env_list,
			here_list);
...
void	close_pipe(t_p_data p_data, t_word_list **word_list,
		t_env_list **env_list, t_here_list **here_list)
{
	if (p_data.i == 0 && close(p_data.pipe_2darr[0]) < 0)
	{
		ft_perror("close");
		free_list_pipe2darr_exit(&p_data.pipe_2darr, word_list, env_list,
			here_list);
	}
	else if (p_data.i == p_data.cnt && close(p_data.pipe_2darr[p_data.cnt]) < 0)
	{
		ft_perror("close");
		free_list_pipe2darr_exit(&p_data.pipe_2darr, word_list, env_list,
			here_list);
